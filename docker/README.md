...
`<OSHUB_STORAGE_ROOT>/кэш/артефакты` с пределом 1 ГиБ. В пуле без источников HEAD и ответ
«не изменилось» в хранилище не ходят вовсе; в пуле с источниками хаб по-прежнему спрашивает
хранилище, лежит ли файл: кэш источников вытесняет уборка. Каталог и предел задаются только
...
Частые чтения из базы — пулы, источники пулов, «есть ли учётка», витрина, пакет с его версиями,
провенансом и каналами для выдачи и страниц пакета — хаб держит в памяти в общем бюджете `OSHUB_CACHE_MEMORY_LIMIT_BYTES` (байты, по
умолчанию 128 МиБ). Запись через хаб видна следующему же запросу; запись мимо процесса хаба
(admin-cli, второй процесс) — не позже срока жизни `OSHUB_CACHE_MEMORY_TTL_SEC` (секунды,
по умолчанию 30). Промахи, попадания и решения кэша видны на
...
длительность сама по себе признаком не является: `СервисПрав.МожетЧитать` в дереве тоже
показывает 0,00 мс, но работу делает — и спан у него есть.
