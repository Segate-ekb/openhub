...
Частые чтения из базы — пулы, источники пулов, «есть ли учётка», витрина, снимок пакета для
выдачи — хаб держит в памяти в общем бюджете `OSHUB_CACHE_MEMORY_LIMIT_BYTES` (байты, по
