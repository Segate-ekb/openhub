...
Рантайм OneScript образ ставит себе сам — из официального релиза
(`ARG ONESCRIPT_VERSION`, архив сверяется по `ARG ONESCRIPT_SHA256`), а базой берёт
`runtime-deps` .NET: сборка OneScript самодостаточна, базе остаётся дать ей ICU, OpenSSL
и корневые сертификаты. Готовым образом чужого тега не пользуемся: тег обещает меньше,
чем кажется — под номером выпуска у публикатора встречалась предрелизная сборка без функций,
на которые хаб опирается через свои библиотеки.

Версию рантайма сборка сверяет дважды: с `.ВерсияСреды` из `packagedef` — и объявлением,
и ответом самого `oscript`. Предрелизную сборку не берёт вовсе, выпуск старее требуемого —
тоже, и падает на стадии распаковки с объяснением. Поднять OneScript в образе —
две строки: новый номер в `ARG ONESCRIPT_VERSION` и его контрольная сумма
