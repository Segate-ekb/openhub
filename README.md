...
Для переезда с другого хаба у зеркала есть **режим переноса**: привезённые им версии
становятся здесь своими, как опубликованные в этом хабе.
...
Тег помечает набор версий, а `пакет@тег` отдаёт максимальную из них. Сам хаб тегов
не заводит: три обычных канала — `stable` (релизы), `prerelease` (плюс кандидаты `-rc`)
и `develop` (сборочные и нестабильные) — заводит кнопка в настройках пакета, а пул
при желании делает это новым пакетам сам. Состав тега либо набирается руками, либо
задаётся правилом по версиям — и тогда новая подходящая версия попадает под тег сама.
...
счётчик скачиваний. `.ospx` забирается файлом прямо со страницы — кнопкой-значком в шапке,
у канала и у каждой версии, — и ставится из скачанного командой `opm install -f <файл>`:
закрытому контуру сеть для этого больше не нужна.
...
трассировка OpenTelemetry тремя уровнями — маршрут, служба, обращение к базе, — HTTP API `/api/v1`
на всё, что есть в интерфейсе, вебхуки и подписки на события.
...
| [Теги и каналы](docs/теги-и-каналы.md) | визард каналов, свои каналы, правила и основной тег |
...
в `opm.cfg` — готовую запись хаб печатает прямо на странице пакета. Хаб, которым `opm`
пользуется по умолчанию, настройкой «opm знает этот хаб по умолчанию» снимает эту
подсказку с пакетов основного публичного пула: им запись не нужна.
