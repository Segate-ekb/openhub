...
`.ospx` кладёт `хранилище`. Об исходе заявки заявителю рассказывают `уведомления`.
Адрес издателя проверяет та же защита исходящих адресов, что и у подписок: в хабе она
одна. Расчётом места в пуле пользуется и `зеркала` — второй путь в пул считает по тем же
правилам, что и push.
