...
Процедура КлючиДомаПула()
	...
	Добавить("settings.pool.danger.note", "Удаление пула вместе с его пакетами в этой версии хаба"
		+ " не поддерживается: артефакты, на которые уже ссылаются сборки, нельзя убрать"
		+ " незаметно для потребителей. Чтобы закрыть пул, сделайте его приватным"
		+ " (раздел «Видимость и доступ») и запретите публикацию"
		+ " (раздел «Апстримы»).");
...
Процедура КлючиДомаПакета()
	...
	Добавить("settings.package.tags.note", "Из коробки: %1 — максимальный релиз без пререлиза,"
		+ " %2 — включая пререлизы. Тег на одной версии не двигается сам: ставьте его в меню"
		+ " нужной версии, в разделе «Версии».");
...
Процедура КлючиНастроекПодсистем()
	...
	Добавить("settings.key.oshub-jobs-readme-reindex-hour.label", "Час переиндексации README");
	Добавить("settings.key.oshub-jobs-readme-reindex-hour.hint",
		"В какой час суток (0–23) хаб перестраивает поисковый индекс описаний пакетов.");
