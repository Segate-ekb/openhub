...
Процедура КлючиДомаПула()
	...
	Добавить("settings.pool.mirror-new.migrate.label", "Перенести пакеты из другого хаба");
	Добавить("settings.pool.mirror-new.migrate.hint",
		"Для переезда: привезённое станет здесь своим, как опубликованное в этом хабе");
	Добавить("settings.pool.mirror-new.migrate.note",
		"Перенесённые версии ведут себя как свои: последняя выбирается среди них, плашки"
		+ " «с апстрима» у них нет, уборка места их не вытесняет. Даты у них — момент"
		+ " переноса, автор неизвестен: чужой хаб этих сведений не отдаёт. Признак задаётся"
		+ " только при заведении — у наполненного зеркала перенос задним числом"
		+ " не появляется.");
...
// Главная кабинета «/me» — сводка состояния дел и витрины её блоков.
...
Процедура КлючиДомаХаба()
	...
	Добавить("settings.hub.identity.lead",
		"Как хаб представляется клиентам opm и браузеру: внешний адрес и порт, а также знает"
		+ " ли opm этот хаб по умолчанию.");
...
// Тексты ОДНИ на все настройки: они говорят про класс настройки и источник значения.
...
// Ключ словаря вычисляется из ключа настройки, поэтому список настроек здесь не повторяется.
Процедура КлючиНастроекРеестра()
	...
	Добавить("settings.key.oshub-instance-opm-default-hub.label", "opm знает этот хаб по умолчанию");
	Добавить("settings.key.oshub-instance-opm-default-hub.hint",
		"Включите на хабе, которым opm пользуется без настройки (hub.oscript.io): для пакетов"
		+ " основного публичного пула хаб перестанет предлагать прописать себя в opm.cfg.");
...
// Метки и подсказки ключей подсистем: пароль, подписки, прокси, счётчики, задачи, OTel, S3.
Процедура КлючиНастроекПодсистем()
	...
	Добавить("settings.key.otel-entity-enabled.label", "Телеметрия базы данных");
	Добавить("settings.key.otel-entity-enabled.hint",
		"Спаны и метрики обращений к базе. Пусто — как телеметрия целиком;"
		+ " false выключает их, оставив остальные сигналы.");
	Добавить("settings.key.otel-entity-repository-enabled.label", "Замер вызовов хранилищ");
	Добавить("settings.key.otel-entity-repository-enabled.hint",
		"Длительность вызовов хранилищ сущностей отдельной гистограммой."
		+ " Пусто — как телеметрия целиком.");
	Добавить("settings.key.otel-entity-query-text.label", "Текст запроса в спане");
	Добавить("settings.key.otel-entity-query-text.hint",
		"Класть SQL запроса в атрибут спана. По умолчанию выключено: произвольный"
...
Процедура КлючиПубличныеОбщие()
	...
	// Сбой, до которого не дотянулся ни один обработчик (СтраницаСбояХаба). Та же фраза
	// уезжает в конверт ошибки машинной двери: сбой один, и звучать он обязан одинаково.
	Добавить("public.failure.title", "Сбой хаба");
	Добавить("public.failure.database",
		"База данных хаба не отвечает. Попробуйте ещё раз через минуту.");
	Добавить("public.failure.internal",
		"Хаб не смог ответить на этот запрос. Попробуйте ещё раз чуть позже.");
	Добавить("public.failure.request", "Код запроса");
	Добавить("public.failure.hint",
		"Если сбой повторяется, назовите администратору хаба код запроса —"
		+ " по нему он найдёт причину в журнале.");
	...
	Добавить("public.pool.start.title", "С чего начать");
	Добавить("public.pool.start.publish", "Опубликовать первый пакет");
	Добавить("public.pool.start.migrate", "Перенести пакеты из другого хаба");
...
Процедура КлючиСтраницыПакета()
	...
	// основной публичный пул хаба, который opm знает по умолчанию: прописывать нечего
	Добавить("public.package.install.hint-short.default-hub",
		"Пакет лежит в основном пуле хаба, а opm знает этот хаб по умолчанию: короткой формы"
		+ " достаточно.");
	...
	Добавить("public.package.version.manifest", "в манифесте %1");
...
Процедура КлючиГлавной()
	...
	// без образца opm.cfg выше пояснению продолжать нечего
	Добавить("public.landing.start.install.default-hub", "Устанавливайте пакеты как обычно:");
...
// Ключи «subscriptions.<тип>.*» запрашивает декларация типа действия, а не сам экран.
Процедура КлючиПодписок()
	...
	Добавить("subscriptions.telegram.template.hint",
		...
		+ " {версия}, {время}, {кто}, {автор}, {репозиторий}, {ссылка}, {процент}; другие"
		+ " имена отклоняются при сохранении. {кто} — учётка хаба, github:<логин> или"
		+ " ci:<репозиторий>; {автор} — автор из манифеста версии; {репозиторий} — адрес"
		+ " репозитория из карточки пакета."
		+ " {процент} заполнен только у события заполнения пула.");
	Добавить("subscriptions.telegram.template.placeholder",
		"{событие}: {пакет} {версия} — {ссылка}");

КонецПроцедуры

// Раздел «провайдеры входа» дома «Хаб»: социальный логин заводится формой, а не только env.
Процедура КлючиПровайдеровВхода()
	...
	Добавить("providers.error.address.single-label",
		...
		+ " под чужого издателя.");
	Добавить("providers.error.address.network",
		"Хост «%1» (%2) %3. Провайдер входа по такому адресу недопустим.");
	Добавить("providers.error.address.network.own",
		"Хост «%1» (%2) %3. Внутренний провайдер настраивается переменными окружения"
		+ " OSHUB_OIDC_*, а не формой администратора.");
	...
	// правило «закрыт в странах»: окно на карточке провайдера и его отказы
	Добавить("providers.geo.title", "Страны и доступ: %1");
	Добавить("providers.geo.lead",
		"Из перечисленных стран провайдер закрыт целиком: ни завести учётную запись,"
		+ " ни войти в существующую, ни привязать его в кабинете.");
	Добавить("providers.geo.field", "Закрыт в странах");
	Добавить("providers.geo.field.hint",
		"Коды стран ISO 3166-1 alpha-2 через запятую, например RU, BY. Пусто — правила нет,"
		+ " провайдер открыт всем. Посетитель, страну которого определить не удалось,"
		+ " считается пришедшим из закрытой страны.");
	Добавить("providers.geo.closed", "Закрыт в странах: %1");
	Добавить("providers.geo.closed.more", "Закрыт в странах: %1 и ещё %2");
	Добавить("providers.geo.open", "Открыт посетителям из всех стран.");
	Добавить("providers.geo.link", "Страны и доступ");
	Добавить("providers.geo.accounts.none",
		"Учётных записей, для которых этот провайдер — единственный способ войти, нет.");
	Добавить("providers.geo.accounts.some",
		"Учётных записей, для которых этот провайдер — единственный способ войти: %1.");
	Добавить("providers.geo.fallback.password",
		"Вход по логину и паролю на хабе включён — им они и войдут.");
	Добавить("providers.geo.fallback.recovery",
		"Восстановление пароля работает: ссылка уйдёт письмом на подтверждённый адрес.");
	Добавить("providers.geo.fallback.none",
		"Вход по логину и паролю выключен, восстановление пароля недоступно: из закрытой"
		+ " страны этим учётным записям войти будет нечем, и помочь им сможет только"
		+ " администратор.");
	Добавить("providers.geo.error.code",
		"«%1» — не код страны: ожидаются две латинские буквы действующего кода"
		+ " ISO 3166-1 alpha-2, например RU.");
	Добавить("providers.geo.error.last",
		"Правило не поставлено: «%1» — последний включённый провайдер без правила по стране,"
		+ " а вход по логину и паролю выключен. После правила в хаб не осталось бы ни одной"
		+ " двери. Включите парольный вход либо оставьте без правила другого провайдера.");
	Добавить("providers.geo.error.gate",
		"Правила по стране недоступны: механизм геоправил на этой инсталляции не собран.");
	Добавить("providers.geo.lockout",
		"каждый включённый провайдер закрыт правилом по стране, и после выключения формы"
		+ " посетителю из закрытой страны войти будет нечем. Снимите правило хотя бы"
		+ " с одного провайдера в разделе «Провайдеры входа» (/hub/settings/providers).");

	// отказ посетителю: ни кода страны, ни «вы из такой-то страны» — только чем войти вместо
	Добавить("login.geo.denied",
		"Этот способ входа недоступен из вашего региона.");
	Добавить("login.geo.fallback.password",
		"Войдите по логину и паролю — форма на этой странице.");
	Добавить("login.geo.fallback.recovery",
		"Пароля нет — нажмите «Забыли пароль?»: ссылка для входа придёт на подтверждённый"
		+ " адрес вашей почты.");
	Добавить("login.geo.fallback.other",
		"Либо войдите через другого провайдера из списка ниже.");
	Добавить("login.geo.fallback.none",
		"Других способов входа на этом хабе нет — напишите администратору хаба.");

	// тот же отказ в кабинете: человек уже вошёл, и звать его к форме входа незачем
	Добавить("login.geo.denied.link",
		"Из вашего региона этого провайдера привязать нельзя.");
	Добавить("login.geo.link.fallback.other",
		"Привязать можно другого провайдера из списка ниже.");
	Добавить("login.geo.link.fallback.none",
		"Других провайдеров, доступных из вашего региона, нет.");

	// встроенная база стран там, где администратор правит правило
	Добавить("providers.geo.db.closes.everyone",
		"Пока хаб не может назвать страну ни одного посетителя, правило по стране закрывает"
		+ " провайдера ВСЕМ: посетитель с неопределённой страной считается пришедшим"
		+ " из закрытой. Положите файл базы стран и дождитесь построения индекса.");
	Добавить("providers.geo.db.file",
		"База стран: %1, выпуск от %2 (%3 сут. назад).");
	Добавить("providers.geo.db.stale",
		"Выпуск устарел: DB-IP обновляет базу раз в месяц. Положите свежий файл либо"
		+ " включите обновление из сети.");
	Добавить("providers.geo.db.missing",
		"Файла базы стран нет: ожидается %1. Положите туда CSV-выпуск DB-IP Lite Country"
		+ " (как — в статье «Развёртывание», раздел «База стран посетителей») либо включите"
		+ " обновление из сети настройкой «Обновлять базу стран из сети».");
	Добавить("providers.geo.db.index",
		"Хаб различает страны: %1 (диапазонов в памяти: %2).");
	Добавить("providers.geo.db.building",
		"Индекс перестраивается под %1: правило вступит в силу, когда сборка закончится.");
	Добавить("providers.geo.db.failed",
		"Прошлая сборка индекса не удалась: %1");
	Добавить("providers.geo.db.unknown",
		"Состояние базы стран прочитать не удалось.");

	// какой страной хаб видит ЭТОТ запрос: им администратор проверяет, что хаб вообще
	// видит посетителей, а не адрес обратного прокси
	Добавить("providers.geo.request.known",
		"Страна вашего запроса: %1 (источник: %2).");
	Добавить("providers.geo.request.unwatched",
		"Ваш адрес публичный и ни в одну наблюдаемую страну не попадает — правило"
		+ " вашего запроса не касается.");
	Добавить("providers.geo.request.unknown",
		"Страна вашего запроса не определена — %1. Если хаб стоит за обратным прокси,"
		+ " назовите его сети в oshub.instance.trusted_proxies — иначе хаб видит адрес"
		+ " прокси, а не посетителя, и правило закроет провайдера всем.");

	// условие лицензии встроенной базы стран (DB-IP Lite, CC BY 4.0)
	Добавить("geo.attribution.dbip", "IP Geolocation by DB-IP");

КонецПроцедуры

// Состояние прогона зеркала: подписи кодов, экран дома «Пул» и админский обзор кабинета.
...
// Повторный ключ — исключение: тихая перезапись ставила бы текст в зависимость от порядка строк.
