...
Процедура КлючиПубличныеОбщие()
	...
	Добавить("public.api.title", "Описание API");
	Добавить("public.api.lead",
		"Все машинные адреса этого хаба: параметры, тела, коды отказа и пробный запрос"
		+ " прямо со страницы.");
	Добавить("public.api.machine", "То же описание машиночитаемым файлом: %1");
	Добавить("public.api.probe",
		"Страница живёт на том же адресе, что и API, поэтому пробный запрос уходит"
		+ " с вашей текущей сессией. Отсюда три следствия:");
	Добавить("public.api.probe.read",
		"чтение ответит так, как видит вошедший, — ответ 200 ещё не значит,"
		+ " что ручка открыта анониму;");
	Добавить("public.api.probe.write",
		"изменение хаб отвергнет: сессии без токена CSRF к мутациям не допускаются;");
	Добавить("public.api.probe.token",
		"ответ от имени ключа доступа — кнопка «Authorize» и персональный токен;"
		+ " ответ анониму — приватное окно браузера или выход из учётки.");
	Добавить("public.api.notice", "Движок страницы — Swagger UI, Apache-2.0: %1, %2, %3.");
	Добавить("public.api.notice.deps", "авторство зависимостей");
	Добавить("public.api.method.title", "Метод не разрешён");
	Добавить("public.api.method.body", "Разрешено: %1");
...
Процедура КлючиКаркасаСтраницы()
	...
	Добавить("chrome.top.nav.api", "Описание API");
