...
паролем», чем «где тут политики». Единственное исключение — `общее/`: там лежит то, чем
пользуются сразу несколько способов входа и что ни одному из них не принадлежит.

| Каталог | Что внутри | Где видно |
| --- | --- | --- |
| `общее/` | сквозное для нескольких способов входа: сверка секретов и их хешей (одно правило на ссылки и ключи), сборка ссылки на публичную страницу из внешнего адреса инсталляции | — |
...
| `провайдером/` | вход через внешнего провайдера: пресеты и конфигурация, провайдеры из базы и из окружения, транзакции OIDC, связка «учётка ↔ внешняя личность», автопривязка профиля, правило «закрыт в странах» и оценка того, кого оно оставит без входа | `/login/oidc/{провайдер}` · `/oidc/callback` |
| `на-входе/` | обвязка каждого запроса: контроллер `/api/v1`, определение личности запроса и адреса клиента за доверенным прокси, единый формат отказов | все запросы к хабу |
